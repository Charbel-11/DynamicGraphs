...
struct DynamicGraphDSU {
	...
	void checkConnected(int u, int v) {
		queries.push_back(Query(areConnected, t, t, u, v));
		t++;
	}
...
int main() {
	...
	int n, m;
	cin >> n >> m;
	DynamicGraphDSU dynamicGraph(n);
	for (int i = 0; i < m; i++) {
		string tp; cin >> tp;
		int u, v; cin >> u >> v; u--, v--;
		if (tp == "add") {
			...
			dynamicGraph.cut(u, v);
		}
		else {
			dynamicGraph.checkConnected(u, v);
		}
	}
	dynamicGraph.processAll();
	for (auto ans : dynamicGraph.ans) {
		if (ans > -1) {
			cout << (ans == 0 ? "NO\n" : "YES\n");
		}
	}
